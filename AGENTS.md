...
- `contracts/` — 灵魂绑定 HeroNFT + 可交易 ComboNFT + Worker 签名的 BattleRecorder + 守擂 Arena + Commit-Reveal 约战 DuelHouse
...
| DuelHouse | 待 `script/DeployDuel.s.sol` 广播（`VITE_DUEL_HOUSE_ADDRESS` + Worker `DUEL_HOUSE_ADDRESS`） |
...
- Deep links: `/?screen=library`, `/?screen=ladder`, `/?screen=arena`, and `/?screen=duel&duel=<id>`（约战邀请链接）.
